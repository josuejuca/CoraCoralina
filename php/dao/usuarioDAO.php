...
class UsuarioDAO
{
 ...
 VALUES (?,?,?,?)";
    ...
    public function atualizarStatusEmprestimo($idemprestimo, $status)
    {
        try {
            $sql = "UPDATE emprestimo SET status = ? WHERE idemprestimo = ?";
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindValue(1, $status);
            $stmt->bindValue(2, $idemprestimo);
            $stmt->execute();
            return true;
        } catch (PDOException $exc) {
            echo $exc->getMessage();
...
//     public function selecionaLivrosEmprestados($usuarioId)
// {
//     try {
//         $sql = "SELECT emprestimo.*, acervo.*, livro.*
//                 FROM emprestimo
//                 INNER JOIN acervo ON emprestimo.acervo_idacervo = acervo.idacervo
//                 INNER JOIN livro ON acervo.livro_idlivro = livro.idlivro
//                 WHERE emprestimo.usuario_idusuario = :usuarioId";
//         $stmt = $this->pdo->prepare($sql);
...
// }
    ...
    public function selecionaEmprestimoPorID($idemprestimo)
    {
        try {
            $sql = "SELECT * FROM emprestimo WHERE idemprestimo = ?";
            $stmt = $this->pdo->prepare($sql);
    ...
    public function incrementarQuantidadeAcervo($idacervo)
    {
        try {
            $sql = "UPDATE acervo SET quantidade = quantidade + 1 WHERE idacervo = ?";
            $stmt = $this->pdo->prepare($sql);
